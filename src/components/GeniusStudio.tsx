    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Genius (e.g. Da Vinci, Tesla, Mozart)"
...
function PracticeBox({ adoptionId, onDone }: { adoptionId: string; onDone: () => void }) {
  ...
  return (
    <div className="mt-3 rounded-lg border border-slate-700 bg-slate-900/60 p-3">
      <textarea value={reflection} onChange={(e) => setReflection(e.target.value)} rows={2} placeholder="How faithfully did you reproduce the strategy? What happened?"
      ...
      <button onClick={submit} disabled={busy} className="mt-2 rounded-lg bg-indigo-600 px-3 py-1.5 text-sm disabled:opacity-50">{busy ? "Saving…" : "Save practice"}</button>
    </div>
  );
}
