    ...
    >
      <div className="mt-4 grid grid-cols-1 gap-3">
        <StudioSection title="CBT reframe (Beck ABC)" hint="Map a thought's belief hierarchy and reframe it." open>
          <input className={inputCls} placeholder="Activating event…" value={event} onChange={(e) => setEvent(e.target.value)} />
          <input className={`${inputCls} mt-2`} placeholder="Automatic thought (optional)" value={thought} onChange={(e) => setThought(e.target.value)} />
        ...
        <StudioSection title="Personality driver (L0)" hint="Trace a complaint to its deep driver + loop.">
          <input className={inputCls} placeholder="Surface problem…" value={surface} onChange={(e) => setSurface(e.target.value)} />
        ...
        <StudioSection title="Narrative identity (McAdams)" hint="Classify a life-story reflection.">
          <textarea className={inputCls} rows={3} placeholder="A life-story reflection…" value={story} onChange={(e) => setStory(e.target.value)} />
        ...
        <StudioSection title="State check-in (L2)" hint="Snapshot of current psychological state + a micro-action.">
          <input className={inputCls} placeholder="How are you right now?" value={checkIn} onChange={(e) => setCheckIn(e.target.value)} />
        ...
        <StudioSection title="Decision motive + guidance" hint="Surface the motive behind a pending decision.">
          <input className={inputCls} placeholder="Pending decision…" value={decision} onChange={(e) => setDecision(e.target.value)} />
          <input className={`${inputCls} mt-2`} placeholder="Emotional context (optional)" value={emo} onChange={(e) => setEmo(e.target.value)} />
