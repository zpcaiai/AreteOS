  ...
  return (
    <div>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <Card title={`Knowledge Assets (${assets.length})`}>{assets.length ? assets.map((a) => <p key={a.id} className="text-sm">{a.title} <span className="text-xs text-slate-500">{a.type}</span></p>) : <Empty>None yet.</Empty>}</Card>
      </div>
    </div>
  );
}
